...
DROP POLICY IF EXISTS "Users can insert notes for their leads" ON lead_notes;
DROP POLICY IF EXISTS "Users can update their own notes" ON lead_notes;
DROP POLICY IF EXISTS "Admins can manage all notes" ON lead_notes;

-- Drop existing triggers and functions if they exist
DROP TRIGGER IF EXISTS update_lead_notes_updated_at_trigger ON lead_notes;
DROP TRIGGER IF EXISTS process_note_mentions_trigger ON lead_notes;
DROP FUNCTION IF EXISTS update_lead_notes_updated_at();
DROP FUNCTION IF EXISTS process_note_mentions();
DROP FUNCTION IF EXISTS extract_mentions_from_note(TEXT);
...
DROP INDEX IF EXISTS idx_lead_notes_mentions;
...
CREATE INDEX idx_lead_notes_mentions ON lead_notes USING GIN(mentions);
...
DO $$ 
BEGIN 
  RAISE NOTICE 'Lead notes table created successfully!';
END $$;